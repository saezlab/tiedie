...
class ActivityScores:
    ...
    def findRegulators(network, de_file, min_hub=10, nperms=1000):
        ...
        for tf, score_pval in sorted(results.items(), key=lambda t: t[1][0]):
            print(score_pval)
            # filter on p-value
            if score_pval[1] > 0.05:
                continue
            tfs_heats[tf] = float(score_pval[0])
    ...
    def generateRankings(self, scores):
        ...
        reverse_scores = []
        for gene, score in sorted(
            scores.items(), key=operator.itemgetter(1), reverse=False
        ):
    ...
    def scoreReg(self, pos_query_set, neg_query_set):
        ...
        rs_const = float(
            2.0 * len(self.scores) - (len(pos_query_set) + len(neg_query_set))
        )
        running_sum = 0.0
        ...
        for i in range(0, len(self.list)):
            ...
            if (set == '-' and gene in neg_query_set) or (
                set == '+' and gene in pos_query_set
            ):
